...
class AdvancedSearchService {
  ...
  Future<List<SearchResult>> _searchRides(String query) async {
    try {
      ...
      return mockRides
          .where((ride) => 
              _fuzzyMatch(query, ride['from_location'] as String) ||
              _fuzzyMatch(query, ride['to_location'] as String))
          .map((ride) => SearchResult(
                id: 'ride_${ride['id']}',
                text: '${ride['from_location']} → ${ride['to_location']}',
                subtitle: '${ride['departure_time']} • ${ride['driver_name']} • ${ride['price_per_person']} Kč',
                type: 'ride',
                icon: '🚗',
                confidence: _calculateTextConfidence(query, '${ride['from_location']} ${ride['to_location']}'),
                data: ride,
              ))
          .toList();
    } catch (error) {
      debugPrint('Chyba při hledání jízd: $error');
      return [];
    }
  }
