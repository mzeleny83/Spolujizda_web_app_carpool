  ...
  rides.forEach((ride) => {
    html += `
      <div style="margin: 10px 0; padding: 15px; border: 1px solid #ddd; border-radius: 5px;">
        ...
        <button onclick="window.testChat = function() { alert('Button clicked!'); openChat(${ride.id}, '${ride.driver_name || 'Řidič'}'); }; testChat();" style="background: #4CAF50; color: white; border: none; padding: 8px 16px; border-radius: 4px; cursor: pointer;">💬 Chat s řidičem</button>
      </div>
    `;
  });
