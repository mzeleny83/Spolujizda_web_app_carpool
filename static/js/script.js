...
function openChat(rideId, driverName) {
  try {
    ...
    chatBox.style.cssText = `position: absolute !important; top: 50% !important; left: 50% !important; transform: translate(-50%, -50%) !important; background: white !important; width: ${isMobile ? '90vw' : '400px'} !important; height: ${isMobile ? '80vh' : '500px'} !important; border-radius: 10px !important; padding: 20px !important; box-shadow: 0 4px 20px rgba(0,0,0,0.3) !important;`;
...
async function loadChatMessages(rideId) {
  try {
    ...
    messages.forEach(msg => {
      ...
      div.style.cssText = `margin: 8px 0; padding: 8px; border-radius: 8px; ${isMyMessage ? 'background: #f5f5f5; text-align: left; margin-right: 50px;' : 'background: #e3f2fd; text-align: left; margin-right: 50px;'}`;
      const now = new Date();
